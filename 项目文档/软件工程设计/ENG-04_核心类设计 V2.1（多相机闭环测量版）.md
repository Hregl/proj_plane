...
**策略来源：** ~~SYS-08 §7（接口全文见 §7.6，本文档不重复定义）~~ 〔引用无效·依据待裁决·见 Q-D2〕。

> ⚠ **勘误（2026-09-26 核，Q-D2）**：上列 `SYS-08 §7.x` 引用**无效**。`SYS-08 V2.1` 的 `# 7`
> 是 **TARGET_FOUND 状态**（§7.1 状态说明／§7.2 执行动作／§7.3 输出／§7.4 失败处理），
> 与被引用的"三级超时／失败三分类／次数表／回退预算"**内容完全不同**（节号碰撞）；
> `§7.5`／`§7.6`／`§7.7` **不存在**（悬空）。∴ 相关策略与数值**目前没有冻结依据**，
> 其现行落点在代码与 `config/measurement.yaml`（本批只登记，**不裁定**）。
...
禁止在任何状态类内保存局部重试计数器——计数分散后无法施加全局回退预算（~~SYS-08 §7.4~~ 〔引用无效·依据待裁决·见 Q-D2〕）。

> ⚠ **勘误（2026-09-26 核，Q-D2）**：`SYS-08 V2.1` 的 `§7.4` 是 **TARGET_FOUND 的"失败处理"**
> （"如果：目标质量不足 ⇒ 进入：FAILED。"），**不是**"回退预算"（节号碰撞）。
> ∴ **禁令本身保留**（它是设计约束），但其**引用来源无效**，依据待裁决。
> 详见《SYS-08-§7引用勘误.md》与《待裁决问题汇总》**Q-D2**。
⚠ 本文件经复核**未随之升版**（它不在本批五份升版文档之列）：本处是**引用勘误标记**，
不是设计变更 —— 见 `AircraftPoseSystem/V2.1-C01_架构裁决变更说明.md`。
