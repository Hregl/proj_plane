# AircraftPoseSystem 类型与命名冻结表 V2.3（A1 数据契约版）
...
> 类型与字段的**来源**在本文档，**接口签名**在 SYS-04 V2.4／SYS-06 V2.3。
...
| 1006 | 取帧契约错误或参数非法（**本地判定，未调用 SDK**） | **裁决 C-015**；语义见 §5.29 |
...
**6 显示转换**（SYS-04 V2.4 记调用责任）：`8U = 像素值 >> (validBits − 8)`，
...
### 5.29 取帧结果与诊断出口（V2.3 新增）

    enum class OpStatus
    {
        Unset,            // 默认构造值 = 漏赋值；**不是成功**
        Ok,
        InvalidArgument, ContractViolation, NotImplemented,   // 本地判定，未调用 SDK
        Timeout, NoFrame, NotStarted, Disconnected,           // 设备／取帧状态
        CorruptFrame,     // 调用成功、但取得的帧不可用（**不得**归为 Timeout）
    ...
    struct OperationResult
    {
        ...
        std::optional<SdkFailure> cleanupError; // 停流/释放/关闭等清理失败，单独留存
        bool ok() const { return status == OpStatus::Ok; }
    };
    struct GrabResult : OperationResult {};     // **不存在**第二个状态字段（无 `GrabStatus`）
...
**`sdkError` / `cleanupError` 的冻结语义**（并明确对应哪次调用）：

| 情形 | `status` | `sdkError` / `cleanupError` |
|---|---|---|
| **未调用** SDK（本地参数／格式／契约错误） | 本地类 | 两者**都是 `nullopt`**（不伪造"调用过"） |
| 调用了 SDK 且返回 `IMV_OK` | 先 `Ok`；**随后**检查帧，帧不可用则改 `CorruptFrame` | `{ImvGetFrame, 0}`（`IMV_OK` 如实保留） |
| 调用了 SDK 且失败 | 按映射表归类，归不了则 `SdkError` | `{该调用, 原码}` |
| **已有主失败**，清理也失败 | **保留首因**（`status` 与 `sdkError` 都不变） | 失败进 `cleanupError` |
| **原操作成功、必要清理失败**（如 `GetFrame` 成功而 `ReleaseFrame` 失败） | **整体返回失败**，以**该清理调用**为错误来源（按同一映射表定 `status`） | `sdkError = {ImvReleaseFrame, 原码}` |
...
|---|---|---|---|---|---|
| `Unset` | **默认构造／漏赋值** | 否 | — | 断言；release 中被放过 ⇒ `9004` | **最高**（100） |
| `ContractViolation` | 本地（组合表矛盾、`compactSizeMatches` 为假后仍被交付、`capturedCount` 与帧数不符） | 否 | **不变** | `1006` 取帧契约错误（本地） | 90（正常运行时**不可达** ⇒ 出现即需人看） |
| `InvalidArgument` | 本地（`timeoutMs == 0`、路径／序列号等参数非法） | 否 | **不变** | `1006` | 85（开发期错误，同上） |
...
| `CorruptFrame` | 调用成功但帧不可用 | 否 | **不变** | `1005`（文本点明"SDK 返回的帧不可用"） | 60 |
| `NotImplemented` | 本地（`Mono12Packed`、`FreeRun`、其它未实现格式码） | 否 | **不变** | `1007` | 50 |
| `Timeout` | SDK −119 或**本地预算耗尽**（未调用 SDK） | 否 | **不变** | `1003` | 40 |
| `NoFrame` | 调用成功、`frameInfo.status` 表示**本拍无有效帧** | 否 | **不变** | `1003` | 30 |
...
· ⚠ **本地契约错误不得冒充 SDK 错误**：`ContractViolation`／`InvalidArgument` 的
  `sdkError` 必须是 `nullopt`，应用码是 `1006` 而**不是** `1005`。
...
### 5.32 `CaptureRound` 与 `ChannelGrabRecord`（V2.3 新增）
    ...
    struct ChannelGrabRecord
    {
        ...
        GrabResult      result;         // 取帧结果（含 status 与 sdkError）
...
4. **`camera.yaml` 的键表归 SYS-17 V1.1 管**（本文档只冻结结构体字段）。
...
V2.3 是 **011-A1 数据契约批**（裁决 C-015）的类型侧落地，**共四类改动**。
...
| 文件 | 状态 |
|---|---|
| `ENG-09_类型与命名冻结表_V2.3_A1数据契约版.md` | **唯一有效版本** |
| `archive/ENG-09_类型与命名冻结表_V2.2_E单位换算修正版.md` | 已归档，仅作历史留存，**不再作为依据** |
| `archive/ENG-09_类型与命名冻结表_V2.1_多相机闭环测量版.md` | 已归档，仅作历史留存，**不再作为依据** |

⚠ 引用 ENG-09 时一律指向 **V2.3**。V2.1 的 §5.16 / §9.2 算式**有错**（缺换算因子）；
V2.2 的 §5.27"来源"列与 §9.2 的一条策略**依据失效**（见 §12.1 第四类）。
两版**均不得**再作引用依据。
