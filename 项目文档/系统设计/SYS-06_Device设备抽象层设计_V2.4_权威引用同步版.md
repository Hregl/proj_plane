# SYS-06_Device设备抽象层设计_V2.4（权威引用同步版）
...
V2.4 只做**一件事**：**同步当前的权威引用**。V2.3 的正文（11 方法接口、责任边界、
三字段、键表修正等）**一字未动**，改的只是"以哪一版为准"的**现时声明**与指向它的节号。

**接口面的唯一权威是 `SYS-04 V2.5 §6.1`**（ICD），本文件是**设计层叙述**：
两者若不一致，**以 SYS-04 V2.5 §6.1 为准**。
变量类型（`OpStatus` / `OperationResult` / `GrabResult` / `DeviceIdentity` /
`TriggerModeState` / `CameraTriggerMode`）的权威是 **ENG-09 V2.4 §5.28～§5.31**。

> ⚠ **为什么必须同批改这一句（留痕）**：本句是**当前裁决规则**，不是历史修订记录 ——
> 旧文写"接口唯一权威是 SYS-04 **V2.4**"、类型权威是"ENG-09 **V2.3**"，
> 而两者已分别升 **V2.5**／**V2.4**（旧版均已入 `archive/`）。
> SYS-04 V2.5 恰恰改的就是 `ICameraBackend` 的**调用责任**（§6.2），
> 旧句会把读者**指回语义已被取代的版本**，故同批更新。
> 修订记录里 V2.3 那一行保留"接口权威为 SYS-04 V2.4 §6.1，类型权威为 ENG-09 V2.3"
> —— 那是**当时的事实**，不做事后改写。
...
**一律以 `SYS-04 V2.5 §6.1` 与其 §6.2 为准**，本节不重复。
三条必须在本层说明的**责任边界**：
...
⚠ **V2.3 新增三个字段**（依据 ENG-09 V2.4 §5.5；该节自 V2.3 起存在、V2.4 仍在其原位）：
...
它进入 `CameraConfig`（ENG-09 V2.4 §6.1），但**不进入 Data 层的帧数据** ——
后端类型由装配摘要的**独立一栏**表达，**不得**填进 `ImageFrame` 或
`DeviceIdentity.modelName`（ENG-09 V2.4 §5.30 第 3 条）。
...
本文档作为 AircraftPoseSystem V2.4 多相机闭环测量系统 Device设备抽象层设计基线。
...
**V2.4 未改动**：§2~§18 的全部内容（含 V2.3 批新增的 11 方法接口与三字段）—— 包括 §7 SDK 发现与编译设计、
