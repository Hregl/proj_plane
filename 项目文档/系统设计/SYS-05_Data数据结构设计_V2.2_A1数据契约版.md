# SYS-05_Data数据结构设计_V2.2（A1 数据契约版）
...
V2.2 是 **011-A1 数据契约批**（裁决 **C-015**，2026-09-26）在 Data 层设计文档上的落地。
它只做**一件事**：把 `ImageFrame` 的三个新字段写进 §5.1。

⚠ **类型与字段的唯一权威是 `ENG-09_类型与命名冻结表_V2.3`**，本文件是**设计层叙述**：
字段名、取值、组合合法性、错误码一律以 ENG-09 V2.3 为准；
两者若不一致，**以 ENG-09 V2.3 为准**（见 ENG-09 §1.1「权威性规则」）。
...
> 裁决 C-01 的**并集冻结**（并集含 `exposureTime`）；两处描述的**权威以 ENG-09 V2.3 §5.5 为准**。
...
本文档作为 AircraftPoseSystem V2.2 多相机闭环测量系统 Data层设计基线。
...
**V2.2 未改动**：§2~§4、§6~§25 的全部内容 —— 包括 §22 数据版本管理、
§23 数据流关系、§24 数据禁止事项、§25 设计冻结总结。
