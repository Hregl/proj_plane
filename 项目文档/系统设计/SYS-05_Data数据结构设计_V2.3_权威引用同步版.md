# SYS-05_Data数据结构设计_V2.3（权威引用同步版）
...
V2.3 只做**一件事**：**同步当前的权威引用**。V2.2 的正文（`ImageFrame` 的三个新字段、
九字段未改的说明等）**一字未动**，改的只是 §1.1 与 §5.1 注里"以哪一版为准"的**现时声明**。

⚠ **类型与字段的唯一权威是 `ENG-09_类型与命名冻结表_V2.4_取帧语义与触发读回版`**，
本文件是**设计层叙述**：字段名、取值、组合合法性、错误码一律以 ENG-09 V2.4 为准；
两者若不一致，**以 ENG-09 V2.4 为准**（见 ENG-09 §1.1「权威性规则」）。
∴ 本版**不重复** ENG-09 的契约条文细节，只登记"本文件涉及的部分变了什么"。

> ⚠ **为什么必须同批改这一句（留痕）**：本句是**当前裁决规则**，不是历史修订记录 ——
> 旧文写"以 ENG-09 **V2.3** 为准"，而 ENG-09 已升 **V2.4**（V2.3 已归档）。
> 新旧语义冲突时，旧句会把读者**指回已归档的版本**。故同批改为 V2.4；
> 修订记录里 V2.2 那一行的"类型权威为 ENG-09 V2.3"是**当时的事实**，**保留不动**。
...
> 裁决 C-01 的**并集冻结**（并集含 `exposureTime`）；两处描述的**权威以 ENG-09 V2.4 §5.5 为准**
> （§5.5「`ImageFrame`」在 V2.4 仍然存在，节号未变）。
...
本文档作为 AircraftPoseSystem V2.3 多相机闭环测量系统 Data层设计基线。
...
**V2.3 未改动**：§2~§25 的全部内容（含 V2.2 批新增的 §1.1 与 §5.1 三字段）——
