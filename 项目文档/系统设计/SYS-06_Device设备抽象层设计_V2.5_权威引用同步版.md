# SYS-06_Device设备抽象层设计_V2.5（权威引用同步版）
...
V2.5 只做**一件事**：**再次同步当前的权威引用**。V2.4 的正文（11 方法接口、责任边界、
三字段、键表修正等）**一字未动**，改的只是"以哪一版为准"的**现时声明**与指向它的节号。

**接口面的唯一权威是 `SYS-04 V2.6 §6.1`**（ICD），本文件是**设计层叙述**：
两者若不一致，**以 SYS-04 V2.6 §6.1 为准**（**调用责任**见其 §6.2）。
变量类型（`OpStatus` / `OperationResult` / `GrabResult` / `DeviceIdentity` /
`TriggerModeState` / `CameraTriggerMode`）的权威是 **ENG-09 V2.5 §5.28～§5.31**；
`camera.yaml` 的**键表**归 **SYS-17 V1.3 §5** 管。

> ⚠ **为什么必须同批改这一句（留痕）**：本句是**当前裁决规则**，不是历史修订记录 ——
> V2.4 写的是"接口唯一权威是 SYS-04 **V2.5**"、类型权威是"ENG-09 **V2.4**"，
> 而两者已分别升 **V2.6**／**V2.5**（旧版均已入 `archive/`）。
> SYS-04 V2.6 恰恰又改了 `ICameraBackend` 的**调用责任**（§6.2：清理责任与异常保全），
> 旧句会把读者**指回语义已被取代的版本**，故同批更新。
> 修订记录里 V2.3／V2.4 两行保留它们当时的版本指向 —— 那是**当时的事实**，不做事后改写。

**V2.4 引入的改动清单（保留在册，本版未动）**：V2.4 只把"接口唯一权威"由
SYS-04 V2.4 ⇒ **V2.5**、类型权威由 ENG-09 V2.3 ⇒ **V2.4**、键表归属由 SYS-17 V1.1 ⇒ **V1.2**
（共 6 处），正文与接口面一字未动。

**V2.3 引入的改动清单（保留在册，本版未动）**：
...
⚠ **V2.3 修正（裁决 C-015；V2.4 只更新了指向的版本号）**：本接口由 **6 个**方法变为 **11 个**。
逐方法的契约、`@return` 的逐值语义、三级释放归属、桩的形态
**一律以 `SYS-04 V2.6 §6.1` 与其 §6.2 为准**，本节不重复。
三条必须在本层说明的**责任边界**：
...
⚠ **V2.3 新增三个字段**（依据 ENG-09 V2.5 §5.5；该节自 V2.3 起存在、V2.5 仍在其原位）：
...
它进入 `CameraConfig`（ENG-09 V2.5 §6.1），但**不进入 Data 层的帧数据** ——
后端类型由装配摘要的**独立一栏**表达，**不得**填进 `ImageFrame` 或
`DeviceIdentity.modelName`（ENG-09 V2.5 §5.30 第 3 条）。
...
本文档作为 AircraftPoseSystem V2.5 多相机闭环测量系统 Device设备抽象层设计基线。
...
| **V2.4** | 2026-09-26 | **权威引用同步版** | **只改当前权威声明**：§1.1／§5.1 注／§6.3／§12.1 里指向的版本改为 **SYS-04 V2.5 §6.1／§6.2**、**ENG-09 V2.4 §5.5／§5.28～§5.31／§5.30／§6.1**（旧版均已入 `archive/`，且 SYS-04 V2.5 改的正是 `ICameraBackend` 的调用责任）。⚠ **正文与接口面一字未动**；修订记录里 V2.3 行的版本指向作为**当时的事实保留**。依据＝**C-016**（011-A1 九项缺口定向修复批）的文档一致性要求 |
| **V2.5** | 2026-09-26 | **权威引用同步版** | **只改当前权威声明**：§1.1 里"接口唯一权威是 SYS-04 **V2.5**"⇒ **V2.6**、类型权威"ENG-09 **V2.4** §5.28～§5.31"⇒ **V2.5**、键表归属"SYS-17 **V1.2** §5"⇒ **V1.3**；正文指针同步（§5.1 注 ⇒ **SYS-04 V2.6 §6.1／§6.2**；§6.3 ⇒ **ENG-09 V2.5 §5.5**；§12.1 ⇒ **SYS-17 V1.3 §5**；§12.2 一带 ⇒ **ENG-09 V2.5 §6.1／§5.30**），实测 **共 10 处**。⚠ SYS-04 V2.6 改的仍是 `ICameraBackend` 的**调用责任**（§6.2：清理责任与异常保全），旧指针会把读者**指回已归档版本**。⚠ **正文与接口面一字未动**；修订记录里 V2.3／V2.4 两行的版本指向作为**当时的事实保留**。依据＝**C-01 v1.9**（011-A1 显示职责与清理确认批）的文档一致性要求 |

**V2.5 未改动**：§2~§18 的全部内容（含 V2.3 批新增的 11 方法接口与三字段）—— 包括 §7 SDK 发现与编译设计、
