# SYS-17_系统配置与参数管理设计_V1.1（A1 数据契约版）
...
V1.1 是 **011-A1 数据契约批**（裁决 **C-015**，2026-09-26）在配置管理文档上的落地。
本版只改**键表**：`camera.yaml` 与 `measurement.yaml` 两处。

⚠ **本节（§5 / §9）的键表是本工程的权威键表**；
键的**类型、默认值、校验规则**的权威是 **ENG-09 V2.3 §6.1（`CameraConfig`）
...
本文档作为 AircraftPoseSystem V1.1 系统配置与参数管理设计基线。
...
**V1.1 未改动**：§2~§4、§6~§8、§10~§20 的全部内容 —— 包括配置只读原则、
