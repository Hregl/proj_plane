# SYS-04_ICD接口控制文件_V2.5（失败边界与清理责任版）
...
（类型定义以 **ENG-09 V2.4 §5.28** 为准，本文件不重复冻结字段表）：
...
`pixel_format` / `valid_bits` / `packing` / `declared_byte_order`（ENG-09 V2.4 §5.28）。
...
"帧状态为 0"（正常）与"帧状态未取得"（`nullopt`）**必须可区分**（ENG-09 V2.4 §5.32）。
...
"调用失败"与"调用成功但返回空串"（ENG-09 V2.4 §5.31）。
...
类型定义以 **ENG-09 V2.4 §5.28～§5.31** 为准，本文件只冻结**接口面**。
...
（那会让一次读失败伪装成"模式正确"）。映射表与读回规则见 ENG-09 V2.4 §5.31。
...
ENG-09 V2.4 §5.31）—— 只有一个 `vector<SdkFailure>` 说不清这件事。
...
且 `sdkError == nullopt`（**根本没调用 SDK**，符合 ENG-09 V2.4 §5.29 第一条语义）。
...
见 **ENG-09 V2.4 §5.29 的状态全表**（本文件不重复）。⚠ 其中两条**不得违反**：
...
语义的逐值对应关系（哪个 `OpStatus` 产出哪个码）见 ENG-09 V2.4 §5.29 的状态全表。
...
| 文件 | 状态 |
|---|---|
| `SYS-04_接口控制文件ICD_V2.5_失败边界与清理责任版.md` | **唯一有效版本** |
