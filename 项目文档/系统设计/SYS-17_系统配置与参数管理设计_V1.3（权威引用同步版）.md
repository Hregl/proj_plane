# SYS-17_系统配置与参数管理设计_V1.3（权威引用同步版）
...
V1.3 只做**一件事**：**再次同步当前的权威引用**。V1.2 的全部正文（`camera.yaml`／
`measurement.yaml` 两张键表、§5.3／§5.4／§9.2 的新增小节）**一字未动**，
改的只是"以哪一版为准"的**现时声明**。

⚠ **本节（§5 / §9）的键表是本工程的权威键表**；
键的**类型、默认值、校验规则**的权威是 **ENG-09 V2.5 §6.1（`CameraConfig`）
与 §6.5（`MeasurementConfig`）**，本文件只登记"有哪些键、含义与失败边界"。

> ⚠ **为什么必须同批改这一句（留痕）**：本句是**当前裁决规则**，不是历史修订记录 ——
> V1.2 写的是"权威是 ENG-09 **V2.4** §6.1 / §6.5"，而 ENG-09 已升 **V2.5**（V2.4 已入 `archive/`）。
> 两版在配置键上若有出入，旧句会把读者**指回归档版本**。故同批改为 V2.5
> （§6.1 与 §6.5 两个节号在 V2.5 中原位保留，**已逐节核对**）；
> 修订记录里 V1.1／V1.2 两行的"权威为 ENG-09 V2.x"是**当时的事实**，**保留不动**。
> ⚠ ENG-09 V2.5 改的是 §5.29 的**聚合规则**（判定类别与实际调用历史分别记录），
> **不涉及 §6.1／§6.5** ⇒ 两张键表**无实质冲突**，本版只动指针。
...
本文档作为 AircraftPoseSystem V1.3 系统配置与参数管理设计基线。
...
| **V1.3** | 2026-09-26 | **权威引用同步版** | **只改当前权威声明**：§1.1 里"权威是 ENG-09 **V2.4** §6.1 / §6.5"⇒ **V2.5**（实测 **1 处**；§6.1 与 §6.5 两个节号在 V2.5 中原位保留，已逐节核对）。ENG-09 V2.5 改的是 §5.29 的**聚合规则**，**不涉及**这两节 ⇒ 两张键表**无实质冲突**。⚠ **键表与全部正文一字未动**；修订记录里 V1.1／V1.2 两行的版本指向作为**当时的事实保留**。依据＝**C-01 v1.9**（011-A1 显示职责与清理确认批）的文档一致性要求 |

**V1.3 未改动**：§2~§20 的全部内容（含 V1.1 批新增的 §5.3／§5.4／§9.2）—— 包括配置只读原则、
