...
//
//  依据：SYS-08 §7.1（三级超时）、§7.3（次数与状态级超时）、§7.4〔引用无效·依据待裁决·见 Q-D2〕（回退预算）、
//        §7.6（**接口冻结原文**）、§7.7〔引用无效·依据待裁决·见 Q-D2〕（恢复路径汇总）、§11（状态机约束 6/8）
//        ENG-09 §6.5（MeasurementConfig 的 16 个重试/超时字段）、裁决 C-20
//        ENG-08 §9（Sprint 5 必做项：原清单遗漏本类）
//
//  ┌──────────────────────────────────────────────────────────────────────┐
//  │ 本类是**唯一**持有重试计数器与任务时限的类（SYS-08 §7.6）。            │
//  │ 状态机自身不得保存任何重试计数 —— 见 §7.6〔引用无效·依据待裁决·见 Q-D2〕 约束与 §11 约束 6。          │
//  └──────────────────────────────────────────────────────────────────────┘
//
//  为什么计数必须集中在一个类里（SYS-08 §7.4〔引用无效·依据待裁决·见 Q-D2〕 的论证）：
//  只有"每状态次数上限"时，存在跨状态循环
//      VALIDATE（失败）→ MEASURE_SELECT → CAPTURE → POSE_SOLVE → VALIDATE
//  每转一圈，各状态的独立计数器都被重新计满，循环次数**不受 §7.3〔引用无效·依据待裁决·见 Q-D2〕 约束**。
...
//
//  ⚠ 本类**不做状态转换决策**（§7.6〔引用无效·依据待裁决·见 Q-D2〕 约束 4）：它只回答"是否允许"，
...
public:
    ...
    ///
    /// ⚠ 清零**只能**经由本方法与 beginTask()（§7.6〔引用无效·依据待裁决·见 Q-D2〕 约束 5：
    /// 计数器不因进入新状态而清零，否则跨状态循环就不受 §7.4〔引用无效·依据待裁决·见 Q-D2〕 约束）。
    void reset();
    ...
    ///
    /// ⚠ 每次重试必须改变至少一项输入（§7.3〔引用无效·依据待裁决·见 Q-D2〕 升级规则）：第 1 次重采帧、
...
private:
    ...
    /// 各状态已消耗的尝试次数。**不因进入新状态而清零**（§7.6〔引用无效·依据待裁决·见 Q-D2〕 约束 5）。
