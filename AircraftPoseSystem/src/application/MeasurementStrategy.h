...
//        §7.7〔引用无效·依据待裁决·见 Q-D2〕（恢复路径汇总）、§11（约束 7/8）
...
/// 失败分类（SYS-08 §7.2〔引用无效·依据待裁决·见 Q-D2〕，冻结）。三类的处理方式**完全不同**，
...
/// §7.3〔引用无效·依据待裁决·见 Q-D2〕 的升级规则：第 N 次尝试必须改变哪些输入。
///
/// 存在的理由（§7.3〔引用无效·依据待裁决·见 Q-D2〕 原文）：**重试禁止重复完全相同的输入** ——
...
struct Escalation
{
    ...
    /// 换用**次优相机**（相对上一次使用的相机）。§7.3〔引用无效·依据待裁决·见 Q-D2〕 第 2 次要求。
    bool switchCamera = false;

    /// 放宽算法内部阈值（RANSAC 迭代数、ratio 阈值）。§7.3〔引用无效·依据待裁决·见 Q-D2〕 第 3 次要求。
...
public:
    ...
    /// §7.4〔引用无效·依据待裁决·见 Q-D2〕 给出该规则的数量依据：系统只有 3 台相机，每次回退排除一台，
