...
//
//  依据：SYS-08 §5（各状态的失败/成功去向）、§6（正常流程）、§7.2〔引用无效·依据待裁决·见 Q-D2〕（失败三分类）、
//        §7.3（次数表 + 升级规则）、§7.7〔引用无效·依据待裁决·见 Q-D2〕（恢复路径汇总，冻结表）
//
    ...
    const data::ErrorInfo& deviceError) const
{
    ...
    //
    // ⚠ 单台相机故障**不走这里**：§7.5〔引用无效·依据待裁决·见 Q-D2〕 要求"降级继续"而非失败，
    ...
    case MeasurementState::TARGET_FOUND:
        ...
        // §7.3 给 TARGET_FOUND 的次数上限是 1，而 §7.6〔引用无效·依据待裁决·见 Q-D2〕 约束 2 要求"每次
        ...
        //（例如明确"§7.3〔引用无效·依据待裁决·见 Q-D2〕 的上限 1 只约束状态内重试，不约束回退后的再次
    ...
    data::MeasurementState state) const
{
    ...
    case MeasurementState::CAPTURE:
        ...
        // 该回退受 §7.4〔引用无效·依据待裁决·见 Q-D2〕 预算约束（同边 2 次、总计 4 次），故有界。
    ...
    case MeasurementState::VALIDATE:
        ...
        // 进入 VALIDATE 时 transition() 已先记过一次尝试（§7.6〔引用无效·依据待裁决·见 Q-D2〕 约束 2），
