...
//        §7.4（回退预算）、§7.6（约束 1/2/4/8）、§7.7〔引用无效·依据待裁决·见 Q-D2〕（恢复路径汇总）、
//        §11（状态机约束 1/4/7/8/9）
...
//  │ 每次进入某状态 → 向 RetryManager 上报一次尝试（§7.6〔引用无效·依据待裁决·见 Q-D2〕 约束 2）；        │
//  │ 每次向后转换  → 先经 beginRollback() 批准（§7.6〔引用无效·依据待裁决·见 Q-D2〕 约束 1 / §11 约束 8）。│
//  └──────────────────────────────────────────────────────────────────────┘
//
//  ⚠ 本类**不保存任何重试计数**（§11 约束 6）。若在此处出现
//  `int retryCount_` 之类的成员，跨状态循环（§7.4〔引用无效·依据待裁决·见 Q-D2〕）就不再受全局预算约束，
...
public:
    ...
    ///        状态机的每次转换都必须经它批准（§7.6〔引用无效·依据待裁决·见 Q-D2〕 约束 1/2），
