...
> **当前测试基线（2026-09-26，011-A1 显示职责与清理确认批后）**：
> **11 个套件 347 个用例，0 失败断言 / 0 异常** —— `flowtest` **45** · `RecorderPackageTest` **20** ·
> `RecorderAdapterTest` 7 · `StateMachineTest` 27 · `AlgorithmStageTest` 23 · `PipelineTest` 28 ·
> `DeviceLayerTest` **84** · `OpticalLayerTest` 38 · `PreviewLayerTest` 40 · `AlignmentControllerTest` 21 ·
> `sysinittest` 14。
> ⚠ 本批**净增 9**（`DeviceLayerTest` 78 → **84**：异常保全三条 ＋ 清理确认两条 ＋ 标记值渲染一条；
> `RecorderPackageTest` 17 → **20**：三条**负向拒绝**用例），与新增的 **9 条 `TEST()`** 一一对应，
> **没有把既有用例删掉凑数**。
> ⚠ **运行目录必须是 `AircraftPoseSystem/`**（垫片的 ROOT，**不是** git 顶层）：`RecorderAdapterTest`
> 的 `RepoConfigMatchesTheNamesTheRecorderExpects` 直接读 `config/`，在 `proj_plane/` 下会**红一条**
> （该用例故意**报失败而非静默跳过**）—— 实测：错误目录 1 红、正确目录 0 红。
> ⚠ 上一条基线（同日，九项缺口定向修复批后）是 **11 套件 / 338 用例**，该批**净增 28**
> （设备 / 记录 / 装配侧 24 ＋ 控制器侧 **4**），全部为**新增用例**。
...
>
> ⚠ **"347 个用例通过"的适用范围（2026-09-24 收尾批更正、2026-09-26 四次更新数字，与 §1 一致）**：
> 数字与执行路径必须**分开记账**，一句话说"全部实跑通过"是过宽的：

| 验证内容 | 实际执行路径 |
|---|---|
| 上表 11 套件 / 347 用例 | **GTest 垫片**（`GoogleTest` 未安装，见 §3.6；`/tmp/gtshim/build_tests.sh`） |
...
| Qt5 5.15.8 | ✅ | — |
| OpenCV 4.6.0 | ✅ | — |
| GoogleTest | ❌ 未安装 | `sudo apt install libgtest-dev`，装后重跑 `cmake ..` 即自动启用。⚠ 当前 347 个用例是**临时 GTest 垫片**下跑的，正式 GoogleTest／CTest 验证待完成（见 §1） |
...
> 依据：`SYS-04 V2.6 §5.4`／`§6.2` 与 `ENG-09 V2.5 §2.5`（两文档**同批发布、须成对引用**；
> §5.2 的**内容**在 V2.6／V2.5 中**原位保留**，本行只更新指针）；
> 裁决正文见 [V2.1-C01_架构裁决变更说明.md](V2.1-C01_架构裁决变更说明.md) **C-016**。

### 5.3 异常保全、清理确认与键组按来源（2026-09-26 批三）

**① 异常必须活到调用方。** `ImvCameraBackend::grab()` 的 `catch` 体内三步
（**描述原异常** → **显式清理** → **写诊断**）**各自独立保护**，末尾的 `throw;`
**无条件执行** ⇒ **原异常原样上抛**，二次异常（`bad_alloc` 等）**不得**取代它：

| 步骤 | 保护方式 | 该步失败时 |
|---|---|---|
| 描述原异常 | 独立 `try` | 该步的**描述文本**丢失，异常**照旧上抛** |
| 显式清理 | `FrameLeaseGuard::cleanup()` 声明与定义**都是 `noexcept`** | 释放调用本身抛出 ⇒ 如实返回 `{ImvReleaseFrame, kCallThrewCode}`，**不重试、不上抛** |
| 写诊断 | 独立 `try/catch(...)` 全吞并置位 | 诊断写入失败 ⇒ **不产生第二个外抛异常** |

⚠ **析构兜底是可达的**（2026-09-26 批三更正）：**描述异常时分配失败** ⇒ 抛点在**显式清理之前**
⇒ 显式清理尚未执行 ⇒ 守卫的**析构兜底会运行**。∴ "变异全绿"**只说明现有测试没覆盖**，
**不能证明**该路径不可达；"**释放恰好一次**"这一要求由**另设用例**独立验证，两者不得互相替代。

**② 释放未获确认 ⇒ 停止该路后续采集（判据前移、只认一个字段）。**
"帧缓冲释放未获确认"这**一件事实**此前有**两条相反处置** —— 区别只在于"帧本身好不好"：

| 修复前的路径 | 主状态 | 清理失败落在哪 | 该通道 |
|---|---|---|---|
| 帧检查**通过** ＋ `ReleaseFrame` 失败 | `SdkError` | 主失败即释放失败 | **被禁用** |
| `CorruptFrame` ＋ `ReleaseFrame` 失败 | `CorruptFrame` | `cleanupError` | **继续可用** ⚠ |

现在**统一为一条规则**：`MultiCameraManager::capture()` **先看 `cleanupError.has_value()`**、
判定排在状态分类**之前**，命中即 `available = false`。⚠ **四条禁止**：
**不覆盖首因**（`status`／`sdkError` 一字不动，现场同时看到"本来因为什么失败"与"资源没还回去"）、
**不把所有 −119 映射成 `Timeout`**、**不盲目重复释放**、**自动恢复须先有资源恢复依据**（本批不做）。
措辞只谈**资源**（"缓冲可能没还回去、会被 SDK 内部缓存复用而污染后续帧"），
**不写**"这台相机坏了"。

**③ 结果包元数据键组按数据来源分组。** 六个 **SDK 事实键** —— `sdk_payload_bytes`、
`expected_compact_bytes`、`compact_size_matches`、`sdk_pixel_format_code`、`sdk_padding_x`、
`sdk_padding_y` —— **只在真身路径（`data_source == "raw"`）写出**；回落路径**不写**。
理由：**"缺键"胜过"值不对的键"** —— 缺键一眼看得出没有这项事实，而 `0`／`false`
会被当成**测出来的数字**。⚠ 而**该文件自己的描述**（格式／有效位／打包／对齐／字节序／`image` 尺寸）
**仍取自已校验的 `captureFormat` 与实际写出的 `image`**。
⚠ 消费方若**假定这六个键恒存在**，须改为**按键是否存在判断**。

**④ `kCallThrewCode` 是标记值，不是码。** `data::kCallThrewCode`（`INT32_MIN`）表示
"**调用抛出了异常、没拿到返回码**"：**不得**把 `-2147483648` 当原码念出来。
全仓**只留一处**渲染 —— `data::sdkFailureCodeText()`（码值／措辞）与 `data::sdkFailureText()`：

```
<调用名> 返回 <码>                      # 正常情形
<调用名> 调用抛出异常、无返回码           # 抛出情形：动词**不是**"返回"，且**不带**"调用 "前缀
```

**⑤ 判定类别与实际调用历史分列。** 聚合状态／应用码说的是"**我们怎么判定**"，
`sdkError` 说的是"**实际调用过什么、结果如何**"—— **本地判定不代表此前未调用 SDK**。
∴ `ContractViolation`／`InvalidArgument` 的 `sdkError` **不再是** `nullopt`；管理器
**原样转发**已有诊断（`sdkError`／`cleanupError`／`diagnosis`／`frameStatusRaw` 逐字段），
**不补造调用**：虚拟后端／脚本替身**可能根本没调 SDK**，原来是 `nullopt` 就**仍是** `nullopt`。

> 依据：`SYS-04 V2.6 §6.2` 与 `ENG-09 V2.5 §5.28／§5.29`（两文档**同批发布、须成对引用**）；
> 裁决正文见 [V2.1-C01_架构裁决变更说明.md](V2.1-C01_架构裁决变更说明.md) **C-017**。
...
| **ENG-09** | 类型与命名冻结表（**类型/命名/单位的唯一权威**）；当前版本 **V2.5（清理责任与异常安全版）** —— 在 V2.4 的帧状态来源／`GrabResult` 本次诊断字段／`TriggerFeatureReadback`／结果包 `raw_image`／`kNoDeadlineNs` 之外，**废除"本地判定 ⇒ `sdkError` 必须为 `nullopt`"**（改为**判定类别与实际调用历史分列**）、确立 `cleanupError` 是"资源还回去了没有"的**唯一**字段、**"释放未获确认 ⇒ 停止该路后续采集"**、异常出口**三步各自独立保护**、`data::kCallThrewCode` 是**标记值不是码**、结果包六个 SDK 事实键**按数据来源分组** |
| **ENG-10** | 算法链接口与配置注入设计 |
...
| **SYS-15 §4** | 误差预算合成规则权威 |
| **SYS-04** | 接口控制文件 ICD（接口权威）；当前版本 **V2.6（清理确认与异常保全版）** —— 与 **ENG-09 V2.5 同批发布、须成对引用**；⚠ 该版改的是"**被调用时发生了什么**"（**清理确认的判据与位置**、**异常保全三步**），冻结签名**一个字没动** |
| **SYS-05** | Data 数据结构设计（设计层叙述，**类型与字段的唯一权威是 ENG-09**）；当前版本 **V2.4（权威引用同步版）** —— 只把"以哪一版为准"改指 **ENG-09 V2.5**（实测 4 处），正文一字未动 |
| **SYS-06** | Device 设备抽象层设计（设计层叙述，**接口面的唯一权威是 SYS-04 §6.1**）；当前版本 **V2.5（权威引用同步版）** —— 只把现时声明改指 **SYS-04 V2.6 §6.1／§6.2**、**ENG-09 V2.5**、**SYS-17 V1.3 §5**（实测 10 处），接口面与正文一字未动 |
| **SYS-17** | 系统配置与参数管理设计（**键表的权威在本档 §5／§9**，键的类型／默认值／校验规则权威是 ENG-09 §6.1／§6.5）；当前版本 **V1.3（权威引用同步版）** —— 只把现时声明改指 **ENG-09 V2.5 §6.1／§6.5**（实测 1 处），两张键表一字未动 |
...
> ⚠ **本节第三次被取代**（2026-09-26 批三补）：SYS-04 现为
> **`项目文档/系统设计/SYS-04_接口控制文件ICD_V2.6_清理确认与异常保全版.md`** —— **它才是当前的接口权威**；
> V2.5 已 `cp -p` 归档进同级 `archive/`。本版与 **ENG-09 V2.5（清理责任与异常安全版）同批发布，
> 两者须成对引用**：SYS-04 管"**调用责任**"（清理确认的**判据与位置**、**异常保全三步**），
> ENG-09 管"**类型与命名**"（`kCallThrewCode` 的标记值语义、`cleanupError` 的唯一性、
> 结果包键组**按数据来源分组**）。裁决正文见
> [V2.1-C01_架构裁决变更说明.md](V2.1-C01_架构裁决变更说明.md) **C-017**。

> ⚠ **同批另有三份文档再次升版（"权威引用同步版"）**（2026-09-26 批三）：
> **`SYS-05` V2.3 → V2.4（4 处）**、**`SYS-06` V2.4 → V2.5（10 处）**、**`SYS-17` V1.2 → V1.3（1 处）**；
> 另将 **ENG-09 V2.5 自身**指向 `SYS-06`／`SYS-17` 的 **2 处**一并校正。
> ⚠ 口径与批二一致：升版的原因**不是"内容变了"，而是"当前权威声明过期了"** ——
> 旧版均已入 `archive/`，冲突时会把读者**指回旧版**。⚠ 只有**正文的现时指针**改，
> **历史修订记录里的旧版本号一律保留**（那是当时的事实）。逐处清单见
> [011-A1_修改位置对照表.md](011-A1_修改位置对照表.md) §12.4。
